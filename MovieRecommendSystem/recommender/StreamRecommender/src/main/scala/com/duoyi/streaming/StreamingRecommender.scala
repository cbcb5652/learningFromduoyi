...
object StreamingRecommender {
  ...
  val MONGODB_STREAM_RECS_COLLTCTION = "StreamRecs";
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(MONGODB_MOVIE_RECS_COLLECTION)
    ...
    ratingStream.foreachRDD { rdds =>
      rdds.map { case (uid, mid, score, timestamp) =>
        // 联通的时候这行数据会打印
        println(">>>>>>>>>>>>>>>")

        // 获取当前用户最近的M次电影评分
        val userRentlyRatings = getUserRecentlyRating(MAX_USER_RATINGS_NUM, uid, ConnHelper.jedis)

        //获取电影P最相似的K个电影
        val simMovies = getTopSimMovies(MAX_SIM_MOVIES_NUM, mid, uid, simMoviesMatrixBroadCast.value)

        //计算待选定的推荐优先级
        val streamRecs = computeMovieScores(simMoviesMatrixBroadCast.value,userRentlyRatings,simMovies)

        //将数据保存到MongoDB中
        saveRecsMongoDB(uid,streamRecs)
   ...
   */
  def saveRecsMongoDB(uid:Int,streamRecs:Array[(Int,Double)])(implicit mongoConfig: MongoConfig): Unit ={
    //到StreamRecs的连接
    val streamRecsCollection = ConnHelper.mongoClient(mongoConfig.db)(MONGODB_STREAM_RECS_COLLTCTION)
   ...
   * @param userRentlyRatings   用户最近的k次评分
   * @param topSimMovies        当前电影的最相似的k个电影
   * @return
   */
  def computeMovieScores(simMovies:scala.collection.Map[Int,scala.collection.immutable.Map[Int,Double]],userRentlyRatings:Array[(Int,Double)],topSimMovies:Array[Int]): Array[(Int, Double)] = {
    ...
    for (topSimMovie <- topSimMovies; userRentlyRating <- userRentlyRatings){
      val simScore = getMovieSimScore(simMovies,userRentlyRating._1,topSimMovie)
      if (simScore > 0.6){
        score += ((topSimMovie,simScore * userRentlyRating._2))
        if (userRentlyRating._2 > 3){
   ...
   * @return
   */
  def getTopSimMovies(num: Int, mid: Int, uid: Int, simMovies: scala.collection.Map[Int, scala.collection.Map[Int, Double]])(implicit mongoConfig: MongoConfig) = {
